document.addEventListener('DOMContentLoaded', () => {
  ...
  let energy = parseFloat(localStorage.getItem('energy')) || 100; // Загружаем энергию из localStorage
  const maxEnergy = 100;
  ...
  function updateEnergy() {
    energyFill.style.width = `${(energy / maxEnergy) * 100}%`; // Исправлено для правильного отображения процента
    energyText.textContent = `${energy}/${maxEnergy}`;
    localStorage.setItem('energy', energy.toFixed(2)); // Сохраняем энергию
  }
  ...
  // Функция для увеличения баланса (изменена)
